...
def MultipleRecepXML(NumberList):
    body = ''
...
def buildXML(username, password, recepients, subject, messageContent):
    ...
    '''.format(sub=subject, user=username, pwd=password, content=messageContent, recep=MultipleRecepXML(recepients))
...
recepNumber =input("Enter list of numbers seperated by space:")
listNumbers =recepNumber.split()

# function call to create xml strinh
doc = buildXML(user,pwd,listNumbers,subject,content)
